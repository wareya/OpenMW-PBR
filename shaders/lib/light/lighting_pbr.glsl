...
// whether to guess an AO value or not (simple clamped brightness calculation)
#define PBR_AUTO_AO 1
// make ao affect specularity
// this is physically wrong, but doing it looks better with non-PBR materials than not doing it
#define PBR_SPECULAR_AO_HACK 1
...
vec3 perLightPBR(float alpha, vec3 diffuseColor, vec3 diffuseVertexColor, vec3 ambientColor, vec3 shadowing, vec3 normal, vec3 viewDir, vec3 lightPos, float lightDistance, float radius, float falloff, float standard_falloff, float cutoff, vec3 ambientLightColor, vec3 lightColor, float metallicity, float roughness, float ao, vec3 f0, vec3 f90, bool indoors, inout vec3 ambientBias)
{
    ...
    //baseIncidence = abs(baseIncidence);
#else
...
vec3 perAmbientPBR(vec3 diffuseColor, vec3 ambientColor, vec3 ambientBias, vec3 ambientAdjust, float ao, float metallicity, float roughness, vec3 normal, vec3 viewDir, vec3 f0, vec3 f90)
{
...
#endif
    {
        ...
        float f0_part = clamp(mix(dot_02_3, 0.6 - dot_02_3*0.25, roughness), 0.0, 1.0);
        f0_part = 1.0 - f0_part;
